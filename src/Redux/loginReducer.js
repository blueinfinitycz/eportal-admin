...
const reducer = (state={type:LOGIN_REQUEST, isLogged:null, data: {data:{user: ''}}}, action) => {
    ...
    if(action.type === LOGIN_REQUEST_SUCCESS){
        return {...state, isLogged: action.payload.data.isLogged, data: action.payload, user: action.payload.data.user};
    }

    if(action.type === LOGOUT_REQUEST_SUCCESS){
        return {...state, isLogged: 0};
    }

    if(action.type === LOGIN_REQUEST_FAILURE || LOGOUT_REQUEST_FAILURE) {
        return {...state, isLogged: 0};
    }  
